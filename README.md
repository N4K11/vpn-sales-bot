...
## Крупное обновление — резервный кабинет и адреса доступа

Обновление от `2026-04-15`.

Что изменилось:

- резервный кабинет стал заметно чище и дружелюбнее для пользователя;
- адреса подписки и резервного кабинета больше не требуют отдельных кнопок `Скопировать ...`;
- в Telegram адрес теперь показывается как отдельная кнопка с самим URL: нажимаете на адрес и он копируется;
- в резервном кабинете адреса сделаны карточками: нажимаете по адресу и он копируется сразу;
- отдельные серверные ключи в резервном кабинете тоже копируются по нажатию на сам адрес;
- для рабочего ключа оставлен только полезный action — `Заменить ключ`.

Что важно сделать после обновления:

1. Обновить сервер через `Админ-панель -> Обновления -> Обновить сейчас` или через `docker compose ... up -d`.
2. Открыть бота заново и зайти в `Мой профиль`, чтобы бот показал свежие ссылки и новые кнопки.
3. Старые сообщения с прежними кнопками копирования не использовать: Telegram не перерисовывает уже отправленные экраны задним числом.
4. Если клиент уже хранит старую подписку, удалить её и импортировать заново по свежей ссылке из профиля.
5. Для клиентов вроде `HApp` использовать только `https`-ссылки на рабочем домене или поддомене.

Для корректной работы общей подписки и резервного кабинета на сервере обязательно должны быть заполнены:

```env
PUBLIC_BASE_URL=https://my.subairfall.com
DOMAIN=my.subairfall.com
```

И DNS должен указывать поддомен на IP сервера, например:

```text
my.subairfall.com -> 87.120.84.217
```

Если используется свой домен:

1. создайте `A`-запись поддомена на IP бота;
2. убедитесь, что `80` и `443` открыты снаружи;
3. перезапустите стек, чтобы `Caddy` выпустил сертификат;
4. после этого заново откройте ссылку подписки из профиля.

Если используется временный режим без домена, сначала можно жить на `sslip.io`, но для максимальной совместимости клиентов лучше перевести всё на собственный домен.
