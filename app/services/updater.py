...
class UpdateService:
    async def get_status(self) -> UpdateStatus:
        ...
        if repository:
            try:
                latest_version, latest_revision, latest_commit_message = await self._fetch_remote_release_state(repository, settings.github_default_branch)
    ...
    async def _fetch_remote_release_state(self, repository: str, branch: str) -> tuple[str, str, str]:
        headers = {
        ...
        timeout = aiohttp.ClientTimeout(total=20)
        async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
            sha, message = await self._fetch_remote_commit(session, repository, branch)
            version = await self._fetch_remote_version(session, repository, branch)
        return version, sha, message

    async def _fetch_remote_commit(self, session: aiohttp.ClientSession, repository: str, branch: str) -> tuple[str, str]:
